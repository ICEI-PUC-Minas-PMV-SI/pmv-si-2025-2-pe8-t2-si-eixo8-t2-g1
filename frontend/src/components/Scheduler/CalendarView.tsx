...
import { AgendamentoDto, EnumTipoAtendimento, PerfilDto, PacienteDto } from "@/types/api";
...
type CalendarViewProps = {
  ...
  professionals: PerfilDto[];
  pacientes: PacienteDto[];
  onAppointmentClick: (appointmentId: string | null, date: Date) => void;
};

const CalendarView = ({ appointments, professionals, pacientes, onAppointmentClick }: CalendarViewProps) => {
  ...
  const getAppointmentForSlot = (day: number, hour: number) => {
    ...
    return appointments.find(app => {
      ...
      return appDate.getFullYear() === date.getFullYear() &&
        ...
        (selectedProfessional === "Todos" || app.perfilId === selectedProfessional);
    });
  };

  const getPatientName = (id?: string) => {
    if (!id) return "Desconhecido";
    return pacientes.find(p => p.id === id)?.nomeCompleto || "Paciente não encontrado";
  };

  const getProfessionalName = (id?: string) => {
    if (!id) return "Desconhecido";
    return professionals.find(p => p.id === id)?.nomeCompleto || "Profissional não encontrado";
  };
                  ...
                  return (
                    ...
                    >
                      {appointment ? (
                        ...
                        >
                          <div className="font-medium truncate" title={getPatientName(appointment.pacienteId)}>
                            {getPatientName(appointment.pacienteId)}
                          </div>
                          <div className="text-xs opacity-80 truncate" title={getProfessionalName(appointment.perfilId)}>
                            {getProfessionalName(appointment.perfilId)}
                          </div>
                        </div>
                      ) : (
